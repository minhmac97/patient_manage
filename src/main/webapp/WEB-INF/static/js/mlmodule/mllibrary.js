...
function confirmOKExit(title, message,titlebuttonOK, callbackOK) {
    var confirmModal = $('<div id="dialog_simple" style="margin-top: 20px; line-height: 25px;">');
    confirmModal.attr('title', title)
    confirmModal.append($('<p>').append(message));
    confirmModal.dialog({
...
function alertSmallBoxWithTitle(title, mes, typealert) {
    $.notify(mes, typealert);
    // typealert = (typealert + "").toLowerCase();
    // var color = "#296191";
    // var icon = "fa-thumbs-up";
    // switch (typealert) {
...
function alertSmallBox(mes, typealert) {
    alertSmallBoxWithTitle("THÔNG BÁO", mes, typealert);
}
